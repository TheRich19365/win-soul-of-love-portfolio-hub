...
const projects = [
  {
    ...
    status: "Live",
    description:
      "ระบบอ่านพลังงานประจำวันเชิงสัญลักษณ์ เชื่อมข้อมูลพลังงาน ออร่า และไพ่เข้ากับ AI เพื่อจัดคำอ่าน บทความ และพรอมต์โปสเตอร์ระดับพรีเมียม โดยไม่ฟันธงชะตาชีวิต",
    englishDescription: "AI-powered daily energy reading, aura, card symbolism, and premium poster prompt generator.",
    cta: "เปิด Daily Energy",
  ...
  },
  {
    ...
    supportTitle: "Living Soul Map · v2.0",
    status: "Live Application",
    description:
      "ระบบสะท้อน Pattern จากวันเดือนปีเกิด แยก Core Blueprint ออกจากพลังงานตามช่วงเวลา พร้อม Being Architecture, Current Cycle, Monthly Pulse และ Daily Signal",
    cta: "เปิด Matrix 9",
          ...
          {projects.map((project) => {
            ...
            return (
              <GlassCard key={project.name} className={`ecosystem-card ${project.featured ? "matrix-project-card" : ""} ${project.accent ? `project-${project.accent}` : ""}`}>
                ...
                {project.englishDescription ? <p className="project-english-line">{project.englishDescription}</p> : null}
                {project.tags ? (
